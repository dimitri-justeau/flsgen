...
public class GenerateTest {
    ...
    public void generateLandscape() throws IOException, JsonException, FlsgenException, FactoryException {
        ...
        LandscapeStructure structure = LandscapeStructureFactory.readFromJSON(IOUtils.toString(new FileReader(path)));
        RegularSquareGrid grid = new RegularSquareGrid(200, 200);
        Terrain terrain = new Terrain(grid);
        double[] rasterData = RasterUtils.loadDoubleDataFromRaster(rasterPath, terrain.getGrid());
        ...
        RasterUtils.exportIntRaster(generator.getRasterData(-1), generator.getGrid(), 0, 0, 0.001, "EPSG:4326", temp.toString());
        Files.delete(temp);
    }

    @Test
    public void generateLandscapeWithMask() throws IOException, FlsgenException {
        String path = getClass().getClassLoader().getResource("mask_raster.tif").getPath();
        LandscapeStructure struct = LandscapeStructureFactory.fromRaster(path, new int[] {1}, Neighborhoods.FOUR_CONNECTED);
    ...
    public void testGenerateWithUniformTerrain() throws IOException, FlsgenException {
        ...
        double[] rasterData = RasterUtils.loadDoubleDataFromRaster(terrain_path, t.getGrid());
        t.loadFromData(rasterData);
